...
_volumes = {settings.MODELS_DIR: models_volume, settings.DATA_DIR: data_volume}
_train_kwargs = dict(
    image=image,
    cpu=settings.CPU,
    gpu=settings.TRAIN_GPU_TYPE,
    volumes=_volumes,
...
_infer_kwargs = dict(
    image=image,
    cpu=settings.CPU,
    gpu=settings.INFERENCE_GPU_TYPE,
    volumes=_volumes,
    secrets=[huggingface_secret],
    timeout=settings.TIMEOUT,
)
...
def _configure_ca_bundle():
    """Configure CA bundle paths for HTTPS requests inside the container.

    Priority:
    1. If `DEEPFOREST_CA_BUNDLE` is set, use it.
    2. Otherwise, if standard SSL env vars are already set, keep them.
    3. Otherwise, fall back to certifi's CA bundle.
    """
    ssl_env_vars = ("SSL_CERT_FILE", "REQUESTS_CA_BUNDLE", "CURL_CA_BUNDLE")
    custom_ca_bundle = os.getenv("DEEPFOREST_CA_BUNDLE")
    if custom_ca_bundle:
        for env_var in ssl_env_vars:
            os.environ[env_var] = custom_ca_bundle
        print(f"Using custom CA bundle from DEEPFOREST_CA_BUNDLE: {custom_ca_bundle}")
        return

    if any(os.getenv(env_var) for env_var in ssl_env_vars):
        return
    ...
    for env_var in ssl_env_vars:
        os.environ[env_var] = certifi_bundle
    print(f"Using certifi CA bundle for HTTPS requests: {certifi_bundle}")
...
def _load_model(
    *,
    ...
    config_filepath : path-like, optional
        Path to the JSON file with model configuration, relative to the data
        volume's root. If None, the defaults from DeepForest are used.
...
def retrain_crown_model(
    ...
    **create_trainer_kwargs : dict-like
        ...
        will be used.
    """
    _ = torch.manual_seed(torch_seed)
    ...
    if config_args is None:
        config_args = settings.DEFAULT_CROWN_CONFIG_ARGS
...
def train_crop_model(
    ...
    **create_trainer_kwargs: KwargsType,
    ...
    retrain_if_exists : bool, default False
        ...
        provided as `dst_filepath` already exists and subsequently overwritten.
        If False, no retraining will be done if the checkpoint already exists.
    crop_model_kwargs : dict-like, optional
        Keyword arguments to pass to the model's initialization method. If none
        provided, the value from `settings.DEFAULT_CROP_MODEL_KWARGS` will be
        used.
    **create_trainer_kwargs : dict-like
        Additional keyword arguments to pass to the model's `create_trainer`
        method. If none provided, the value from
        `settings.DEFAULT_CROP_MODEL_CREATE_TRAINER_KWARGS` will be used.
    ...
    skip_existing_crops : bool, default True
        If True, skip writing crops for a split when matching crops already exist
        and were produced from the same annotations.
    ...
    def _compute_annotations_signature(annot_df):
        signature_df = annot_df[
            ["image_path", "xmin", "ymin", "xmax", "ymax", "label"]
        ].copy()
        signature_df["label"] = signature_df["label"].astype(str)
        row_hashes = pd.util.hash_pandas_object(signature_df, index=False).to_numpy()
        return hashlib.sha256(row_hashes.tobytes()).hexdigest()

    def _expected_counts_by_label(annot_df):
        counts = annot_df["label"].astype(str).value_counts(sort=False).to_dict()
        return {str(label): int(count) for label, count in counts.items()}

    def _has_required_crops(dst_dir, expected_counts):
        for label, expected_count in expected_counts.items():
            label_dir = path.join(dst_dir, label)
            if not path.isdir(label_dir):
                return False
            try:
                existing_count = sum(
                    1 for entry in os.scandir(label_dir) if entry.is_file()
                )
            except OSError:
                return False
    ...
    def _load_crops_manifest(manifest_filepath):
        try:
    ...
    def _save_crops_manifest(manifest_filepath, *, signature, expected_counts):
        payload = {"signature": signature, "expected_counts": expected_counts}
    ...
    def write_crops(annot_df, dst_dir):
        ...
        expected_counts = _expected_counts_by_label(annot_df)
        annotations_signature = _compute_annotations_signature(annot_df)
        manifest_filepath = path.join(dst_dir, ".crops_manifest.json")
        manifest = _load_crops_manifest(manifest_filepath)
        if (
            skip_existing_crops
            and manifest is not None
            and manifest.get("signature") == annotations_signature
            and manifest.get("expected_counts") == expected_counts
            and _has_required_crops(dst_dir, expected_counts)
        ):
            print(f"Skipping crop writing at {dst_dir} (cached crops found).")
            return
        ...
        _save_crops_manifest(
            manifest_filepath,
            signature=annotations_signature,
    ...
    with tmp_ctx:
        ...
        crop_model.load_from_disk(
            train_dir=train_dir,
            val_dir=test_dir if test_df is not None else None,
        )
        start_time = time.time()
...
@app.function(**_infer_kwargs)
def predict(
    *,
    img_filepaths: str | Sequence[str] | None = None,
    remote_img_dir: str | None = None,
    ...
    """Predict tree crown bounding boxes using a DeepForest-like model.

    Parameters
    ----------
    img_filepaths : str or list-like of str, optional
        Image filepaths or URLs to predict on. If URLs are provided, the images are
        downloaded to `remote_img_dir` before prediction. If not provided, both
        `img_filenames` and `remote_img_dir` must be provided.
    remote_img_dir : str, optional
        Path to the remote directory with images, relative to the data volume's
        root. If `img_filepaths` is provided and `remote_img_dir` is not, a cache
        directory is used.
    img_filenames : str or list-like of str, optional
        Images to predict on, relative to `remote_img_dir`. If an empty sequence is
        provided, all images in `remote_img_dir` with the specified `img_ext` will
        be used.
    img_ext : str, optional
        ...
        `settings.DEFAULT_IMG_EXT` will be used.
    checkpoint_filepath : path-like, optional
        Path to the checkpoint file to load from the model volume (relative to the
        volume's root). If None, the model from `model_name` and `model_revision`
        are loaded from Hugging Face Hub.
    ...
    patch_size : int, optional
        ...
        provided, the default value from `settings.DEFAULT_PATCH_SIZE` will be
        used. If the image is smaller than `patch_size`, it will be processed as a
        whole.
    ...
    if remote_img_dir is None:
        # use pooch default cache
        # TODO: accept args for project-based cache?
        remote_img_dir = pooch.os_cache("pooch")
    # append data volume prefix
    remote_img_dir = path.join(settings.DATA_DIR, remote_img_dir)
    if img_filepaths is not None:
        if urlparse(img_filepaths[0]).scheme:
            img_filepaths = [
                pooch.retrieve(
                    img_url,
                    known_hash=None,
                    # override default file name which includes a hash so that the
                    # returned data frame doesn't
                    fname=path.basename(img_url),
                    path=remote_img_dir,
                )
                for img_url in tqdm(img_filepaths)
            ]

    else:
        if img_filenames is None or remote_img_dir is None:
            raise ValueError(
                "If `img_filepaths` is not provided, both `img_filenames` and "
                "`remote_img_dir` must be provided."
            )
        ...
        img_filepaths = [
            path.join(remote_img_dir, img_filename) for img_filename in img_filenames
        ]
...
def checkpoint_to_hf_hub(
    ...
    **push_to_hub_kwargs,
    ...
    model_card_kwargs : dict, optional
        ...
        argument in the underlying Hugging Face model's `save_pretrained` method.
    **push_to_hub_kwargs
        Additional keyword arguments passed to the underlying Hugging Face model's
        `save_pretrained` method.
    """
    _checkpoint_filepath = path.join(settings.MODELS_DIR, checkpoint_filepath)
    model = _load_model(checkpoint_filepath=checkpoint_filepath)
    print(f"Loaded model from checkpoint: {_checkpoint_filepath}")
    _push_to_hub_kwargs = dict(push_to_hub_kwargs)
    token = _push_to_hub_kwargs.pop("token", os.environ["HF_TOKEN"])
    with tempfile.TemporaryDirectory() as tmp_dir:
...
def upload_file(
    ...
    *,
    ...
    remote_dir : str, optional
        Directory in the data volume where the file should be uploaded. Ignored if
        `remote_filepath` is provided. If not provided, `remote_filepath` must be
        provided.
