...
def plot_metric(
    ...
    **plot_kwargs,
    ...
    """Plot the evolution of a metric from a Comet experiment.

    Parameters
    ----------
    experiment : comet_ml.APIExperiment-like
        Experiment object exposing ``get_metrics(metric_name)``.
    metric_name : str
        Name of the metric to plot.
    ax : matplotlib.axes.Axes, optional
        Axis where the metric is plotted. If None, matplotlib creates one.
    x : str, default "step"
        Metric field to use for the x-axis.
    **plot_kwargs
    ...
    matplotlib.axes.Axes
        Axis containing the metric plot.
    """
    ...
    if legend:
        ...
        if "cropmodel_label" in pred_gdf.columns:
            pred_gdf = pred_gdf.assign(
                **{"label": pred_gdf["cropmodel_label"].astype(int)}
            )
